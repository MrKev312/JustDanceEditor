...
internal static class LegacyCinematicFxTemplateReader
{
    ...
    public static bool TryRead(
        ...
        out LegacyCinematicFxTemplate template)
    {
        template = default!;
        if (texturePaths.Count == 0)
            return false;

        IReadOnlyList<LegacyCinematicParticleTemplate> particleTemplates =
            LegacyCinematicParticleTemplateReader.ReadAll(bytes, allowEmptyPhaseList: true);
        ...
        IReadOnlyList<LegacyCinematicFxControlTemplate> controls = ReadFxControls(bytes, out uint triggerFxNameId, out uint defaultFxNameId);
        IReadOnlyDictionary<uint, LegacyCinematicFxControlTemplate> controlsByNameId = controls.ToDictionary(control => control.NameId);
        List<LegacyCinematicFxEmitterTemplate> emitters = new(emitterCount);
        string? fallbackMaterialPath = materialPaths.Count > 0 ? materialPaths[0] : null;
        for (int index = 0; index < emitterCount; index++)
        {
            LegacyCinematicParticleTemplate particleTemplate = particleTemplates[index];
            ...
            emitters.Add(new LegacyCinematicFxEmitterTemplate(
                index,
        ...
        template = new LegacyCinematicFxTemplate(emitters, controls, controlsByNameId, defaultFxNameId, triggerFxNameId);
        return true;
    }

    private static void ApplyFxControlRenderPriorities(
        List<LegacyCinematicFxEmitterTemplate> emitters,
        IReadOnlyList<LegacyCinematicFxControlTemplate> controls)
    {
        ...
        for (int index = 0; index < emitters.Count; index++)
        {
            ...
            if (LegacyCinematicFxIds.IsValid(descriptorNameId) &&
        ...
        // FXControllerComponent_Template::onTemplateLoaded() neutralizes particle
        // Z sort by assigning renderPrio in FXControl particle-list order:
        // index - params.positionOffset.z, with index increasing by 0.001.
        float renderPriorityIndex = 0.0f;
        foreach (LegacyCinematicFxControlTemplate control in controls)
        {
            foreach (uint particleNameId in control.ParticleNameIds)
            {
                ...
                LegacyCinematicFxEmitterTemplate emitter = emitters[emitterIndex];
                LegacyCinematicParticleTemplate particleTemplate = emitter.ParticleTemplate;
                LegacyCinematicParticleParameters parameters = particleTemplate.Parameters;
    ...
    private static bool TryReadDescriptorNameId(byte[] bytes, int parameterOffset, out uint nameId)
    {
        ...
        if (!LegacyCinematicFxIds.IsValid(value))
            return false;
    ...
    private static IReadOnlyList<LegacyCinematicFxControlTemplate> ReadFxControls(
        byte[] bytes,
        out uint triggerFxNameId,
        out uint defaultFxNameId)
    {
        triggerFxNameId = LegacyCinematicFxIds.Invalid;
        defaultFxNameId = LegacyCinematicFxIds.Invalid;
        int componentOffset = FindUInt32(bytes, LegacyBinarySerializer.GetTypeId<LegacyCinematicFxControllerComponentTemplateBinary>());
        ...
        List<LegacyCinematicFxControlTemplate> controls = [];
        ...
        out int endOffset)
    {
        endOffset = offset;
        int cursor = offset;
        List<LegacyCinematicFxControlTemplate> parsed = [];
        ...
        for (int index = 0; index < count; index++)
        {
            ...
            if (!LegacyCinematicFxIds.IsValid(nameId))
                return false;
            ...
            parsed.Add(new LegacyCinematicFxControlTemplate(
                nameId,
                stopOnEndAnim,
                playOnce,
                emitFromBase,
    ...
    private static bool TryReadIdList(byte[] bytes, ref int cursor, int componentEnd, out IReadOnlyList<uint> ids)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            cursor += 4;
            if (LegacyCinematicFxIds.IsValid(id))
                values.Add(id);
        }
    ...
    {
        int fxBankOffset = FindUInt32(bytes, LegacyBinarySerializer.GetTypeId<LegacyCinematicFxBankComponentTemplateBinary>(), startOffset);
        int particleOffset = FindUInt32(bytes, LegacyBinarySerializer.GetTypeId<LegacyCinematicParticleGeneratorComponentTemplateBinary>(), startOffset);
        int next = -1;
        if (fxBankOffset >= 0)
            next = fxBankOffset;
