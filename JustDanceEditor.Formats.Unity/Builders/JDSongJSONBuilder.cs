...
public class JDSongJSONBuilder
{
    ...
    public static JDCacheSong CreateSong(SongDatabaseEntry songData, uint cacheNumber, string coverFilename, string coachesSmallFilename, string coachesLargeFilename, string audioPreviewOpusFilename, string videoPreviewMidVp9WebmFilename, string audioOpusFilename, string videoHighVp9WebmFilename, string mapPackageFilename, string? songTitleLogoFilename = null, Guid guid = default)
    {
        if (guid == Guid.Empty)
            guid = Guid.NewGuid();

        JDCacheSong song = new()
        {
