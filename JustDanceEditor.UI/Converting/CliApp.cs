    ...
    ILogger<CliApp> logger)
{
    ...
    private static int PrintHelp()
    {
        ...
        Examples:
          JustDanceEditor.UI convert --input C:\Songs\HighHopes --target nx-2022 --output C:\Out
          JustDanceEditor.UI convert --input C:\UnitySong --target uncooked --output C:\Out --source Unity
          JustDanceEditor.UI batch --input C:\Bundles --target durango-2022 --output C:\Out
          JustDanceEditor.UI targets
        """);
        return 0;
    }
    ...
    private int ListTargets(CliArguments options)
    {
        ...
        foreach (PlatformDescriptor platform in ConversionTargetSelector.GetSortedPlatforms(targets))
        {
            ...
            foreach (ConversionTargetDefinition target in ConversionTargetSelector.GetSortedTargetsForPlatform(targets, platform.PlatformCode))
                Console.WriteLine($"  {target.TargetCode,-24} {ConversionTargetSelector.FormatTargetLabel(target)}");
        }
    ...
    private int ConvertBatch(CliArguments options)
    {
        ...
        foreach (BatchInput input in inputs)
        {
            try
            {
                converted += ConvertBatchInput(input, target, targetFormat, targetStrategy, answers, outputPath, existingSongs, downloadOnlineAssets);
            }
            catch (Exception ex)
            {
                failed++;
        ...
        Console.WriteLine($"Batch complete. Converted: {converted}. Failed/skipped: {failed}.");
        return failed == 0 ? 0 : 1;
    }

    private int ConvertBatchInput(
        BatchInput input,
        ...
        bool downloadOnlineAssets)
    {
        ...
        try
        {
            ...
            return ConvertOneBatchSong(input.SourceFormat, targetFormat, targetStrategy, importRequest, input.Path, outputPath, target, answers, existingSongs, downloadOnlineAssets) ? 1 : 0;
        }
        catch (MultipleConversionItemsFoundException ex)
        {
            int converted = 0;
