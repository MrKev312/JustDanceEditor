...
public class SsdDecoder : IImageDecoder
{
        ...
        where TPixel : unmanaged, IPixel<TPixel>
    {
        ...
        try
        {
            ...
            if (stream.Read(header, 0, header.Length) == header.Length &&
                header[0] == 0x00 && header[1] == 0x00 && header[2] == 0x00 && header[3] == 0x09 &&
                header[4] == (byte)'T' && header[5] == (byte)'E' && header[6] == (byte)'X')
            {
                if (stream.CanSeek)
            ...
            Image<Bgra32> decoded = SSD.GetImage(stream);

            return typeof(TPixel) == typeof(Bgra32)
                ? (Image<TPixel>)(object)decoded
                : decoded.CloneAs<TPixel>();
        }
        finally
        {
            if (stream.CanSeek)
