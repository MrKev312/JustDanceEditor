...
internal static class UnityAssetMaterializer
{
    ...
    private static bool CopyAllVideoVariants(string sourceFolder, string destinationFolder)
    {
        ...
        string[] allowedExtensions = [".webm", ".mp4", ".mkv", ".mov"]; // extendable if formats evolve
    ...
    private static string ComposeRenderKey(AssetTypeValueField keyField)
    {
        ...
        for (int i = 0; i < 4; i++)
            parts[i] = keyField[$"data[{i}]"].AsUInt;
    ...
    private static void TryDeleteDirectory(string folder)
    {
        ...
        {
            // Swallow cleanup errors; empty directories are harmless.
        }
    }
